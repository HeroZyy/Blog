<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="${comment}" prop="bclass">
        <el-input
    ...
    <el-table v-loading="loading" :data="teaList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="${comment}" align="center" prop="img"/>
      <el-table-column label="${comment}" align="center" prop="bclass"/>
      <el-table-column label="施肥方法" align="center" prop="column1"/>
      <el-table-column label="修剪技术" align="center" prop="column2"/>
      <el-table-column label="采摘时机" align="center" prop="column3"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="${comment}" prop="img">
          <el-input v-model="form.img" type="textarea" placeholder="请输入内容"/>
        </el-form-item>
        <el-form-item label="${comment}" prop="bclass">
          <el-input v-model="form.bclass" placeholder="请输入${comment}"/>
        </el-form-item>
        <el-form-item label="施肥方法" prop="column1">
          <el-input v-model="form.column1" type="textarea" placeholder="请输入内容"/>
        </el-form-item>
        <el-form-item label="修剪技术" prop="column2">
          <el-input v-model="form.column2" type="textarea" placeholder="请输入内容"/>
        </el-form-item>
        <el-form-item label="采摘时机" prop="column3">
...
<script>
import { listTea, getTea, delTea, addTea, updateTea } from '@/api/system/tea'
import Vue from 'vue'
...
export default {
  ...
  methods: {
    ...
    //   })
    //
    //   Vue.prototype.$globalVariable = response.rows
    //   // console.log("分类 ： ", Vue.prototype.$globalVariable); // 在这里打印
    // },
    /** 查询tea列表 */
    getList() {
      this.loading = true
      listTea(this.queryParams).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            ...
            addTea(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除tea编号为"' + ids + '"的数据项？').then(function() {
        return delTea(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/tea/export', {
        ...this.queryParams
      }, `tea_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
