...
//            Assert.isTrue(temp.getUserId().longValue() == ShiroUtil.getProfile().getId().longValue(), "没有权限编辑");
    ...
    public CommonResult blogsClass(String bclass,Integer currentPage) {
        ...
        if(bclass == null || currentPage < 1) {
            pageData = blogService.page(page, new QueryWrapper<MBlog>().orderByDesc("gmt_create"));
        }
        else {
            pageData = blogService.page(page, new QueryWrapper<MBlog>().eq("bclass", bclass).orderByDesc("gmt_create"));
        }
        return CommonResult.successResponse(pageData,"获取成功");
    }
